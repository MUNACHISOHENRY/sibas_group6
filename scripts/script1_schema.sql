...
--   1. Drops the existing 'sibas' database if it already exists (clean slate)
--   2. Creates a fresh 'sibas' database
--   3. Connects to it
--   4. Creates all tables, in dependency order, with every required constraint
--   5. Seeds the minimum data needed to log in and start using the app
--      (one system-settings row, and one default administrator account)
--
-- HOW TO RUN IT  (from a terminal, while connected to your own user account):
--     psql -U postgres -f script1_schema.sql
--
-- HOW TO VERIFY AFTERWARDS (inside psql, once connected to sibas):
--     \dt          -- lists every table; you should see 11 of them
--     \d student   -- inspects one table's columns + constraints
--
-- DEFAULT LOGIN AFTER RUNNING THIS SCRIPT
--     username: admin
--     password: admin123        <-- change this from the app on first login
--
-- DESIGN NOTES
--   - All primary keys are SERIAL (auto-incrementing integers).
--   - No arrays, no JSON, no JSONB. Pure normalised tables, as required.
--   - Schema is in 3NF: a student's department is reached via their
--     programme (STUDENT -> PROGRAMME -> DEPARTMENT), so we do NOT store
--     department_id on the student row -- that would be a transitive
--     dependency.
--   - Foreign keys use ON DELETE rules deliberately:
--       * RESTRICT  : block the delete if anything still references it
--                     (e.g. you can't delete a department that has programmes)
--       * CASCADE   : when the parent goes, the child rows go too
--                     (e.g. deleting an app_user removes its student row)
-- =============================================================================
